...
# https://mayerkrebs.com/building-a-backtesting-framework-in-python-from-scratch/
# https://mayerkrebs.com/building-a-backtesting-framework-in-python-part-ii/
...
class Engine():
            ...
            if order.side == 'buy' and self.cash >= self.data.loc[self.current_idx]['Open'] * order.size:
                if order.type == 'limit':
                    ...
                    if order.limit_price >= self.data.loc[self.current_idx]['Low']:
                        ...
                        can_fill = True
                        # print(self.current_idx, 'Buy Filled. ', "limit", order.limit_price, " / low",
                ...
                if order.type == 'limit':
                    ...
                    # else:
                    #     print(self.current_idx, 'Sell NOT filled. ', "limit", order.limit_price, " / high",
                    #           self.data.loc[self.current_idx]['High'])
                else:
                    can_fill = True
    ...
    def _get_stats(self):
        ...
        metrics = {}
        total_return = 100 * ((self.data.loc[self.current_idx][
                                   'Close'] * self.strategy.position_size + self.cash) / self.initial_cash - 1)
        ...
        metrics['returns_annualized'] = ((p.iloc[-1] / p.iloc[0]) ** (
                    1 / ((p.index[-1] - p.index[0]).days / 365)) - 1) * 100
        p_bh = portfolio_bh
        metrics['returns_bh_annualized'] = ((p_bh.iloc[-1] / p_bh.iloc[0]) ** (
                    1 / ((p_bh.index[-1] - p_bh.index[0]).days / 365)) - 1) * 100
        ...
        metrics['sharpe_ratio_bh'] = (metrics['returns_bh_annualized'] - self.risk_free_rate) / metrics[
            'volatility_bh_ann']
